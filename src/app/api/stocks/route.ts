...
export async function GET(request: NextRequest) {
  try {
    ...
    // Try Yahoo Finance India for Indian stocks
    if (symbol.includes('.BSE') || symbol.includes('.NS') || symbol.includes('.BO')) {
      try {
        const yahooData = await fetchYahooFinanceIndia(symbol, period);
        if (yahooData) {
          return NextResponse.json(yahooData);
        }
      } catch (error) {
        console.log('Yahoo Finance India failed:', error);
      }
    }
...
// Yahoo Finance India API function
async function fetchYahooFinanceIndia(symbol: string, period: string) {
  try {
    ...
    console.log('Trying Yahoo Finance India with symbol:', yahooSymbol);
    ...
    if (!response.ok) {
      throw new Error(`Yahoo Finance India returned ${response.status}`);
    }
    
    const data = await response.json();
    
    if (!data.chart || !data.chart.result || data.chart.result.length === 0) {
      throw new Error('No data available from Yahoo Finance India');
    }
    ...
    console.error('Yahoo Finance India error:', error);
    throw error;
  }
}
...
function generateMockStockData(symbol: string, period: string) {
  ...
  for (let i = days; i >= 0; i--) {
    ...
    currentPrice = Math.max(50, currentPrice + dailyChange);
    
    historicalData.push({
      date: date.toISOString().split('T')[0],
      open: Number((currentPrice + (Math.random() - 0.5) * 5).toFixed(2)),
